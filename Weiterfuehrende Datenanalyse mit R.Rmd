---
title: "Weiterführende Datenanalyse mit R"
subtitle: "Unterlage zum Workshop auf den Dozententagen am 30.01.2017"
author: "Prof. Dr. Joachim Schwarz"
date: ''
output:
  pdf_document:
    fig_height: 3
...
\newline
\newline
__RStudio und RCommander__  
Falls Sie RStudio benutzen, dann öffnen Sie zur Eingabe der Skriptbefehle bitte ein neues R-Skriptfenster (File-->New File-->R Script). Falls Sie den RCommander benutzen, können Sie sämtliche Skriptbefehle in das Skriptfenster (oberes Fenster) eingeben.

__Pakete aktivieren__  
Erforderlich sind die Pakete \texttt{mosaic, vcd, gplot, effects, multcomp, lmtest, car}. Sämtliche Pakete werden mit \texttt{library(paketname)} bzw. \texttt{require(paketname)} eingebunden, Beispiel: \texttt{library(mosaic)}.  
Falls dabei die Fehlermeldung "package does not exist" erscheint, muss das Paket zunächst mit \texttt{install.packages(\dq paketname\dq, dependencies=TRUE)} installiert werden, Beispiel: \texttt{install.packages(\dq gplot\dq, dependencies=TRUE)}. Für die Installation ist eine Internetverbindung erforderlich. Nach der Installation muss der library-Befehl zur Aktivierung ausgeführt werden.
...
__Daten einlesen__  
Datensätze können auf verschiedene Arten eingelesen werden. Der Trinkgelddatensatz ist bereits in R enthalten, im Paket reshape2. Falls die Fehlermeldung erscheint, dass das Paket \texttt{reshape2} nicht existiert, so muss dies vorab installiert werden, vgl. hierzu die obigen Ausführungen. 
```{r}
...
__Beispiel (Trinkgeld-Datensatz)__
Es ist zu prüfen \emph{$H_{0}:$ Die Höhe der Restaurantrechnung hat keinen Einfluss auf die Höhe des Trinkgelds ($H_0: \beta_1 = 0$).}  
...
\begin{itemize}
\item Linearität der Beziehung zwischen den unabhängigen und der abhängigen Variablen.
\item  Normalverteilung der Residuen.
\item Homoskedastizität (konstante Varianz) der Residuen.
\item Keine Autokorrelation der Residuen.
\item Keine Multikollinearität der unabhängigen Variablen.
\item Robustheit (Keine Ausreißer).
\end{itemize}
...
\emph{$H_{01}:$ Die Höhe der Restaurantrechnung hat keinen Einfluss auf die Höhe des Trinkgelds ($H_0: \beta_1 = 0$).}   
\emph{$H_{02}:$ Die Gruppengröße hat keinen Einfluss auf die Höhe des Trinkgelds ($H_0: \beta_2 = 0$).}  
